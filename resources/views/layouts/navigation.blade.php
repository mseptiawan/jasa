<nav x-data="{ open: false }" class="sticky top-0 z-40 bg-white/90 backdrop-blur-sm w-full border-b border-gray-200">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        ...
                        @if(in_array(auth()->user()->role, ['customer', 'seller']))
                            <x-nav-link :href="route('orders.index')" :active="request()->routeIs('orders.*')">
                                ...
                                @if(auth()->user()->role === 'seller')
                                    @php
                                        $pendingCount = \App\Models\Order::whereHas('service', fn($q) => $q->where('user_id', auth()->id()))
                                            ->where('status', 'pending')->count();
                                    @endphp
                                    @if ($pendingCount > 0)
                                        <span class="ml-2 bg-red-500 text-white text-xs font-bold rounded-full px-2 py-0.5">{{ $pendingCount }}</span>
                                    @endif
                                @endif
                            </x-nav-link>
                            ...
                            <x-nav-link :href="route('services.favorites')" :active="request()->routeIs('services.favorites')">
                                {{ __('Favorit Saya') }}
                            </x-nav-link>
                        @endif
                        ...
                        @if (auth()->user()->role === 'customer')
                            @php
                                $hasPendingApplication = \App\Models\ProviderApplication::where('user_id', auth()->id())->where('status', 'pending')->exists();
                            @endphp
                            @if($hasPendingApplication)
                                <x-nav-link :href="route('provider.applications')" :active="request()->routeIs('provider.applications')">
                                    {{ __('Pengajuan Saya') }}
                                </x-nav-link>
                            @else
                                <x-nav-link :href="route('service.apply')" :active="request()->routeIs('service.apply')">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    ...
                    <div x-data="{ open: false }" class="relative me-4">
                        <button @click="open = !open" class="relative p-2 rounded-full hover:bg-gray-200 focus:outline-none focus:bg-gray-200 transition">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                            ...
                            <div class="max-h-96 overflow-y-auto">
                                @forelse(auth()->user()->unreadNotifications as $notif)
                                    <a href="{{ $notif->data['url'] ?? '#' }}" class="block px-4 py-3 hover:bg-gray-100 transition-colors">
                                        <div class="font-semibold text-sm text-gray-800">{{ $notif->data['title'] ?? 'Notifikasi' }}</div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            ...
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
             ...
             @auth
                ...
                @if(in_array(auth()->user()->role, ['customer', 'seller']))
                    ...
                    <x-responsive-nav-link :href="route('services.favorites')" :active="request()->routeIs('services.favorites')">
                        {{ __('Favorit Saya') }}
                    </x-responsive-nav-link>
                @endif
                ...
                @if (auth()->user()->role === 'customer')
                     <div class="border-t border-gray-200 pt-2 mt-2">
                        @php $hasPendingApplication = \App\Models\ProviderApplication::where('user_id', auth()->id())->where('status', 'pending')->exists(); @endphp
                        @if($hasPendingApplication)
                            <x-responsive-nav-link :href="route('provider.applications')">{{ __('Pengajuan Saya') }}</x-responsive-nav-link>
                        @else
                            <x-responsive-nav-link :href="route('service.apply')">{{ __('Jadi Penyedia Jasa') }}</x-responsive-nav-link>
                        @endif
                    </div>
                @endif
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800">{{ optional(Auth::user())->full_name ?? 'Guest' }}</div>

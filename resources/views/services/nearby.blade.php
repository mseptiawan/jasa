<x-app-layout>
    ...
    <div class="container mx-auto p-4 md:p-8 max-w-7xl">
        ...
        <h2 class="text-2xl font-bold mb-6 text-gray-900">Hasil Pencarian</h2>
        ...
        {{-- START: Grid Container (Grid 2 kolom di mobile) --}}
        <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($services as $service)
                ...
                <div
                    {{-- HAPUS shadow-md dan pastikan border/background tetap ada --}}
                    class="bg-white rounded-lg border border-gray-200 overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 relative">
                    {{-- Gambar utama --}}
                    @if ($mainImage)
    ...
    <script>
        // Logika sederhana untuk tombol "Cari Layanan Terdekat" (btn-nearby)
        document.getElementById('btn-nearby').addEventListener('click', () => {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition((position) => {
                    const lat = position.coords.latitude;
                    const lng = position.coords.longitude;
                    // Mengarahkan ke rute services.nearby dengan koordinat yang didapat

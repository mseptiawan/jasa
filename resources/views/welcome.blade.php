        ...
        <div class="max-w-7xl mx-auto flex flex-wrap justify-between items-center py-3 px-4 sm:px-6 lg:px-8 gap-4">
            ...
            <div class="w-full md:w-auto md:flex-1 md:max-w-md lg:max-w-lg order-3 md:order-2">
                 <form action="{{ route('home') }}" method="GET" class="relative">
                    <input type="text" name="search" placeholder="Cari layanan apa pun..." value="{{ request('search') }}" class="w-full py-2 pl-4 pr-10 border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-primary focus:border-transparent transition" autocomplete="off">
            ...
            <div class="flex items-center space-x-8 order-2 md:order-3">
                <a href="{{ route('login') }}" class="animated-underline text-gray-700 hover:text-primary font-semibold transition-colors">Login</a>
                <a href="{{ route('register') }}" class="animated-underline text-accent hover:text-yellow-400 font-semibold transition-colors">Register</a>
            </div>
        </div>
    </header>
        ...
        <main>
                ...
                <section class="mb-12">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach ($highlightServices as $service)
                            ...
                            @endphp
                            <div class="bg-white rounded-lg overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 border-2 border-accent">
                                <div class="relative">
                                    <a href="{{ route('services.show', $service->slug) }}">
                                        @if ($mainImage)
                                            <img src="{{ asset('storage/' . $mainImage) }}" alt="{{ $service->title }}" class="w-full h-40 object-cover">
                                        @else
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex items-center gap-2 text-sm text-gray-600 mb-3">
                                        @if ($profilePhoto)
                                            <img src="{{ asset('storage/' . $profilePhoto) }}" alt="{{ $service->user->full_name }}" class="w-7 h-7 rounded-full object-cover">
                                        @else
            ...
            <section>
                <h2 class="text-2xl font-bold mb-5 text-gray-900">Semua Layanan</h2>
                 <div id="normal-services-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @forelse($normalServices as $service)
                        @php
                            ...
                            $profilePhoto = $service->user->profile_photo ?? null;
                        @endphp
                        <div class="normal-service-card bg-white rounded-lg border border-gray-200 overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
                             <a href="{{ route('services.show', $service->slug) }}">
                                @if ($mainImage)
                                    <img src="{{ asset('storage/' . $mainImage) }}" alt="{{ $service->title }}" class="w-full h-40 object-cover">
                                @else
                ...
                <!-- Loading Indicator -->
                <div id="loading-indicator" class="w-full flex justify-center items-center py-8 gap-3 text-gray-600 hidden">
